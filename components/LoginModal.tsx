...
import type { MockUser, Platform } from '@/types';
...
const LoginModal = ({ open, onClose, onLogin }: LoginModalProps) => {
  const [loginType, setLoginType] = useState<'streamer' | 'mod'>('streamer');
  const [selectedStreamer, setSelectedStreamer] = useState('');
  const [modName, setModName] = useState('');

  const handleStreamerLogin = (platform: Platform) => {
    onLogin({
      type: 'streamer',
      name: 'YourChannel',
      platform,
      verified: platform === 'twitch' || platform === 'kick',
      mods: [],
    });
  };
      ...
      <DialogContent className="bg-slate-800 border-white/10 text-white sm:max-w-sm">
        ...
        {loginType === 'streamer' ? (
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-500 text-center">Mod status verified via platform API</p>
          </div>
        )}

        <p className="text-xs text-gray-500 text-center">Demo mode - all logins simulated</p>
      </DialogContent>
    </Dialog>
  );
};
