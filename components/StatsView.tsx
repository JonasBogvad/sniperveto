...
const StatsView = ({ reports }: StatsViewProps) => {
  const totalReports = reports.reduce((acc, r) => acc + r.reportCount, 0);
  const uniqueSnipers = reports.length;

  const gameBreakdown = reports.reduce<Record<string, number>>((acc, r) => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-6 sm:mb-8">
        ...
        <div className="bg-slate-800/50 border border-white/10 rounded-xl p-3 sm:p-4 text-center">
          <div className="text-lg sm:text-2xl font-bold text-yellow-400 mb-1 truncate">
            {mostReportedGame?.[0] ?? '-'}
          </div>
